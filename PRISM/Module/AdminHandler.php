...
class AdminHandler extends SectionHandler
{
    ...
    /**
     * Reloads the config files and connects to the hosts.
     * 
     * @return bool
     */
    public function init()
    {
        ...
        if ($this->loadIniFile($this->_admins)) {
            ...
            //include_once ROOTPATH . '/modules/prism_interactive.php'; // use this at the top
            Interactive::queryAdmins($this->_admins);
...
ININOTES;
            if ($this->createIniFile('Admins Configuration File', $this->_admins, $extraInfo)) {
                console('Generated config/'.$this->iniFile);
            }
        }

        // Read account vars to verify / maybe generate password hashes
        foreach ($this->_admins as $username => &$details) {
            // Convert password?
    ...
    public function &getAdminsInfo()
    {
        ...
        foreach ($this->_admins as $user => $details) {
            $info[$user] = array(
    ...
    /**
     * Get information about a single user account
     * 
     * @param string &$username The users handle.
     * 
     * @return array
     */
    public function getAdminInfo(&$username)
    {
        ...
        if (!isset($this->_admins[$username])) {
            return false;
        }

        return array(
            'accessFlags' => $this->_admins[$username]['accessFlags'],
            'connection' => $this->_admins[$username]['connection'],
            'temporary' => isset($this->_admins[$username]['temporary']) ? $this->_admins[$username]['temporary'] : false,
        );
    }
    ...
    public function getRealmDigest(&$username)
    {
        ...
        return $this->_admins[$username]['realmDigest'];
    }

    /**
     * Check if admin exists
     * 
     * @param string &$username Admin Handle
     * 
     * @return bool
     */
    ...
    {
        $username = strToLower($username);
        return isset($this->_admins[$username]);
    }

    /**
     * Verify username and password.
     * 
     * @param string &$username Users Handle
    ...
    public function isPasswordCorrect(&$username, &$password)
    {
        ...
        return (
            isset($this->_admins[$username]) &&
            $this->_admins[$username]['password'] == sha1($password.$PRISM->config->cvars['secToken'])
        );
    }
     ...
     * @param int    $accessFlags User Privledges
     * @param string $connection  I'm not sure what this is...
     * @param bool   $store       I'm also not sure what this is...
     * 
     * @return bool
     */
    public function addAccount($username, $password, $accessFlags = 0, $connection = '', $store = true)
    {
        ...
        // Add new user to $this->_admins
        $this->_admins[$username] = array(
        ...
        if ($store) {
            $this->appendSection($username, $this->_admins[$username]);
        } else {
            $this->_admins[$username]['temporary'] = true;
        }
    ...
    public function makePermanent($username)
    {
        ...
        if (!isset($this->_admins[$username]) || !isset($this->_admins[$username]['temporary']) || !$this->_admins[$username]['temporary']) {
            return false;
        }

        unset($this->_admins[$username]['temporary']);
        $this->appendSection($username, $this->_admins[$username]);
    ...
    public function deleteAccount($username, $store = true)
    {
        ...
        // Remove the account from $this->_admins
        unset($this->_admins[$username]);
    ...
    public function changePassword($username, $password, $store = true)
    {
        ...
        // Update the password in $this->_admins
        $this->_admins[$username]['realmDigest'] = md5($username.':'.HTTP_AUTH_REALM.':'.$password);
        $this->_admins[$username]['password'] = sha1($password.$PRISM->config->cvars['secToken']);

        // Rewrite password and realmDigest lines for user in admins.ini
        if ($store) {
            $this->rewriteLine($username, 'password', $this->_admins[$username]['password']);
            $this->rewriteLine($username, 'realmDigest', $this->_admins[$username]['realmDigest']);
        }
    ...
    public function setAccessFlags($username, $flags, $store = true)
    {
        ...
        // Set the permissions
        $this->_admins[$username]['accessFlags'] = $flags;

        // Rewrite accessFlags line for user in admins.ini
        if ($store) {
            $this->rewriteLine($username, 'accessFlags', flagsToString($this->_admins[$username]['accessFlags']));
        }
