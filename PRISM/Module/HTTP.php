...
use PRISM\Module\SectionHandler as SectionHandler;
use PRISM\Module\PHPParser as PHPParser;

namespace PRISM\Module;

class HTTP extends SectionHandler
{
...
class HttpClient
{
    ...
    public function handleInput(&$data, &$errNo)
    {
        ...
        if (file_exists($this->http->getDocRoot().$scriptName)) {
            ...
            } elseif (is_dir($this->http->getDocRoot().$this->httpRequest->SERVER['SCRIPT_NAME'])) {
    ...
    private function validateAuthorization()
    {
        ...
        } elseif (preg_match('/^Basic (.*)$/', $this->httpRequest->SERVER['HTTP_AUTHORIZATION'], $matches)) {
            // Basic method
            ...
            if ($pos === false) {
                ...
                if (!$this->hasRequestUri) {
                    ...
                    if ($len > HTTP_MAX_URI_LENGTH) {
                        $this->errNo = 414;
                        return false;
                    } elseif ($len > 3 && !preg_match('/^(GET|POST|HEAD).*$/', $this->rawInput)) {
                        $this->errNo = 444;
                        return false;
                    }
                }
