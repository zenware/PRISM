 ...
 * @link       https://github.com/zenware/PRISM/blob/devel/Plugin/Admin.php
 */

namespace PRISM\Plugin;

/**
 * Admin functions for PRISM
 *
 * @category   Plugin
 * @package    PRISM
 * @subpackage Plugin\Admin
...
class Admin extends Plugins
{
    ...
    public function cmdPluginLoad($cmd, $ucid)
    {
        ...
        if (($argc = count($argv = str_getcsv($cmd, ' '))) < 4) {
            ...
            $PluginsAll = get_dir_structure(PRISM::ROOTPATH . '/plugins/');
...
/*            foreach ($PluginsNotloaded as $Plugin) {
                if (validatePHPFile(PRISM::ROOTPATH . '/plugins/' . $Plugins . '.php'))
...
/*            $MTC->Text('You can unload any of the following plugins.')->Send();
    ...
    {
        IS_MTC()->UCID($ucid)->Text('PRISM Version ^7' . PRISM::VERSION)->Send();
        return PLUGIN_HANDLED;
    }
